...
export default function Sidebar({ activeTabId, onOpenFile, collapsed, onToggleCollapse }) {
  const { t } = useTranslation();

  return (
    <aside
      className={
        'flex h-full bg-surface-container-low shrink-0 transition-[width] duration-200 ' +
        (collapsed ? 'w-12' : 'w-64')
      }
    >
      <div className="w-12 bg-surface flex flex-col items-center py-4 space-y-2 shrink-0">
        <button
          type="button"
        ...
        {collapsed && FILES.map((f) => {
          ...
          return (
            ...
            >
              <span className={'material-symbols-outlined ' + (active ? 'text-primary' : f.iconClass)}>
                {f.icon}
              </span>
            </button>
          );
        ...
        <div className="mt-auto">
          <a
            ...
            rel="noreferrer"
            title="resume.pdf"
            className="p-2 block text-on-surface-variant opacity-70 hover:opacity-100 hover:bg-surface-container-high hover:text-primary cursor-pointer"
          >
      ...
      {!collapsed && (
        <div className="flex-1 flex flex-col overflow-y-auto min-w-0">
          <div className="px-4 py-3 flex items-center justify-between">
            <span className="font-label text-xs uppercase font-semibold tracking-widest text-primary truncate">
            ...
            {FILES.map((f) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="font-label text-sm truncate">{f.label}</span>
                </button>
              );
            })}
          </div>
